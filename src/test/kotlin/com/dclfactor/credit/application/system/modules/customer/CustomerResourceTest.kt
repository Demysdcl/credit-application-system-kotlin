...
//                    .andExpect(MockMvcResultMatchers.jsonPath("$.id").value(1))
    ...
    fun `should find customer by id and return 200 status`() {
        ...
        customerRepository.save(builderCustomerDto().toEntity())
            .also {
                ...
                //then
                mockMvc.perform(
                    MockMvcRequestBuilders.get("$URL/${it.id}")
    ...
    fun `should not find customer with invalid id and return 404 status`() {
        ...
        mockMvc.perform(
            ...
            .andExpect(MockMvcResultMatchers.jsonPath("$.title").value("Not Found! Consult the documentation"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.timestamp").exists())
            .andExpect(MockMvcResultMatchers.jsonPath("$.status").value(404))
            .andExpect(
                MockMvcResultMatchers.jsonPath("$.exception")
                    .value("class com.dclfactor.credit.application.system.modules.exception.BusinessException")
            )
            .andExpect(MockMvcResultMatchers.jsonPath("$.details[*]").isNotEmpty)
            .andDo(MockMvcResultHandlers.print())
    }
    ...
    fun `should delete customer by id and return 204 status`() {
        ...
        customerRepository.save(builderCustomerDto().toEntity())
            .also {
                ...
                //then
                mockMvc.perform(
                    MockMvcRequestBuilders.delete("$URL/${it.id}")
                        .accept(MediaType.APPLICATION_JSON)
                )
    ...
    fun `should not delete customer by id and return 404 status`() {
        ...
        Random().nextLong().also {
            ...
            //then
            mockMvc.perform(
                MockMvcRequestBuilders.delete("$URL/${it}")
                    .accept(MediaType.APPLICATION_JSON)
            )
                .andExpect(MockMvcResultMatchers.status().isNotFound)
                .andExpect(MockMvcResultMatchers.jsonPath("$.title").value("Not Found! Consult the documentation"))
    ...
    fun `should update a customer and return 200 status`() {
        ...
        customerRepository.save(builderCustomerDto().toEntity())
            .also {
                ...
                //then
                mockMvc.perform(
                    MockMvcRequestBuilders.patch("$URL?customerId=${it.id}")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(builderCustomerUpdateDto()))
                )
                    .andExpect(MockMvcResultMatchers.status().isOk)
